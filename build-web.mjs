...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .save .n { color: #54d98c; }
  .panel { background: #161922; border: 1px solid #2a2f3a; border-radius: 12px; padding: 16px; margin: 18px 0; }
...
<main>
  ...
  <section class="panel" aria-labelledby="wrap-h">
    <h2 id="wrap-h">Browser helper</h2>
    <p class="sub">Choose the CLI you plan to use, shrink the prompt, then copy the compressed version with a short next-step note. This page prepares the prompt; the local proxy wrapper is the interception path.</p>
    <div class="row">
      <div>
        <label for="wrap-target" style="margin:0 0 6px">Target CLI</label>
        <select id="wrap-target">
          <option value="copilot" selected>GitHub Copilot CLI</option>
          <option value="claude">Claude Code</option>
          <option value="codex">OpenAI Codex CLI</option>
        </select>
      </div>
      <button id="copywrap" type="button" class="secondary">Copy wrapper prompt</button>
      <button id="copywrapnote" type="button" class="secondary">Copy next step</button>
      <span id="wrapcopied" role="status" hidden></span>
    </div>
    <p id="wrapnote" class="sub" style="margin-top:10px"></p>
  </section>
  ...
  <footer>
    ...
    <p><strong>Personal project:</strong> TokenCodec is the personal work of the author and is not affiliated with, endorsed by, or supported by Microsoft Corporation or any of its subsidiaries.</p>
...
const WRAP_TARGETS = {
  copilot: {
    ...
    note: 'Open GitHub Copilot CLI, paste the compressed prompt, then press Enter.',
  },
  claude: {
    name: 'Claude Code',
    note: 'Open Claude Code, paste the compressed prompt, then press Enter.',
  },
  codex: {
    name: 'OpenAI Codex CLI',
    note: 'Open OpenAI Codex CLI, paste the compressed prompt, then press Enter.',
  },
};
...
function renderWrapper() {
  const target = WRAP_TARGETS[$('wrap-target').value] || WRAP_TARGETS.copilot;
  const hasInput = $('prompt').value.trim().length > 0;
  $('wrapnote').textContent = hasInput
    ? target.note + ' The button below copies the same compressed prompt that the optimizer shows.'
    : target.note + ' Paste a prompt below to generate the compressed version.';
}
...
$('wrap-target').addEventListener('change', renderWrapper);
...
$('copywrap').addEventListener('click', () => copyText($('out').value, $('wrapcopied')));
$('copywrapnote').addEventListener('click', () => {
  const target = WRAP_TARGETS[$('wrap-target').value] || WRAP_TARGETS.copilot;
  copyText(target.note, $('wrapcopied'));
});
